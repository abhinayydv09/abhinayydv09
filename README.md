<h1 align="center">Abhinay Kumar Yadav</h1>

<p align="center">
<b>Data Scientist • Data Engineer • AI Engineer • Data Analyst</b>
</p>

<p align="center">
Designing scalable data pipelines, machine learning systems, and AI applications that transform data into actionable insights.
</p>

<p align="center">
<a href="mailto:abhinaykyadav09@gmail.com">Email</a> •
<a href="https://www.linkedin.com/in/abhinayy09/">LinkedIn</a> •
<a href="https://github.com/abhinayydv09">GitHub</a>
</p>

---

## Profile

Artificial Intelligence graduate with a strong foundation in data analytics, machine learning, deep learning, and data engineering.

My work focuses on building end-to-end solutions—from data acquisition and ETL pipelines to predictive modeling, API development, and deployment.

Currently seeking opportunities where I can contribute to building reliable, scalable, and data-driven products.

---

## Core Expertise

**Programming**  Python • SQL

**Data Analytics**  Pandas • NumPy • Power BI • Excel

**Machine Learning**  Scikit-learn • TensorFlow • PyTorch • LightGBM

**Data Engineering**  PySpark • FastAPI • Flask • PostgreSQL • MySQL • ETL

**Artificial Intelligence**  Large Language Models • NLP • Computer Vision • OpenCV

**Cloud & Development**  Azure ML • Git • GitHub

---

## Selected Projects

| Project | Focus |
|----------|-------|
| **Deep Learning Stock Price Prediction** | Time Series Forecasting, Portfolio Optimization |
| **SQL LLM Chatbot** | Natural Language to SQL, FastAPI, LLM |
| **FinPulse** | Financial News Summarization, NLP |
| **Sign Language Recognition** | Computer Vision, Deep Learning |
| **Mental Health Risk Prediction** | Predictive Analytics, Classification |

---

## Highlights

- IEEE Published Research Author
- Best Capstone Project Award – Praxis Tech School
- B.Tech in Artificial Intelligence
- Post Graduate Program in Data Science

---

## GitHub Statistics

<p align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=abhinayydv09&show_icons=true&hide_border=true&theme=github_dark"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=abhinayydv09&layout=compact&hide_border=true&theme=github_dark"/>

</p>

---

## Contact

**Email**  abhinaykyadav09@gmail.com

**LinkedIn**  https://linkedin.com/in/abhinayy09

**GitHub**  https://github.com/abhinayydv09
